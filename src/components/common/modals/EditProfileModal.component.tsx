...
interface EditProfileDialogProps {
  ...
  onSave?: (
    data: ProfileForm & { avatarFile?: File | null }
  ) => Promise<void> | void;
  isOpen: boolean;
  onClose: () => void;
}
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error("Invalid file type", {
        description: "Please select a PNG, JPG, or WebP image file.",
      });
      return;
    }
        ...
        <form onSubmit={handleSubmit(submit)} className="space-y-6 mt-4">
          <div className="flex items-center gap-2">
            ...
            >
              {avatarPreview ? (
                ...
                <AvatarFallback>
                  {(defaultValues?.firstName ?? "?").charAt(0)}
                </AvatarFallback>
              )}
            </Avatar>
          ...
          <DialogFooter className="pt-4">
            <div className="flex items-center justify-end gap-2 w-full">
              ...
              <Button
                type="submit"
                className="bg-blue-500 hover:bg-white hover:border-blue-500 hover:text-blue-500 hover:border-[0.5px]"
                disabled={isSubmitting}
              >
